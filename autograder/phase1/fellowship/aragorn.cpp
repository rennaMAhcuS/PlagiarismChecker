...
struct DADAC {
    ...
    int hod;
    DADAC() : dugc(0), hod(0) {
        std::cout << "DADAC constructor with dugc = " << dugc << " hod = " << hod << std::endl;
    }
    DADAC(int __dugc, int __hod) : dugc(__dugc), hod(__hod) {
        std::cout << "DADAC constructor with dugc = " << dugc << " hod = " << hod << std::endl;
    }
    virtual ~DADAC() {
        std::cout << "DADAC destructor with dugc = " << dugc << " hod = " << hod << std::endl;
    }
};
...
struct ADAC : virtual public DADAC {
    int hod;
    int dean;
    ADAC() : DADAC(0, 0), hod(0), dean(0) {
        std::cout << "ADAC constructor with hod = " << hod << " dean = " << dean << std::endl;
    }
    ADAC(int __hod, int __dean) : DADAC(0, 0), hod(__hod), dean(__dean) {
        std::cout << "ADAC constructor with hod = " << hod << " dean = " << dean << std::endl;
    }
    ADAC(int __dugc, int __hod_dept, int __hod, int __dean) :
        DADAC(__dugc, __hod_dept), hod(__hod), dean(__dean) {
        std::cout << "ADAC constructor with hod = " << hod << " dean = " << dean << std::endl;
    }
    ~ADAC() {
        std::cout << "ADAC destructor with hod = " << hod << " dean = " << dean << std::endl;
    }
};
